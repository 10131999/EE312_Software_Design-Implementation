...
double sqrtIt(double x, double low_guess, double high_guess) {
   ...
   while ((low_guess * low_guess) < x){
       low_guess += 1;
   }
  high_guess = low_guess;
   while ((high_guess * high_guess) > x){
       high_guess -= .00001;
   }
    return high_guess;
}
...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    if ((high_guess * high_guess) > x){
         return sqrtRec(x, low_guess, high_guess - 1);
    }
    return high_guess;
...
int strCompare2(char* str1, char* str2) {
    ...
    if (*str1 == *str2 || *str2 - 32 == *str1 || *str1 - 32 == *str2){
...
Martian change(int cents) {
    ...
    if (cents%12 <= 2){
        int cents12 = cents - cents%12;
        p1.dodeks += cents12/12;
        cents = cents - (p1.dodeks * 12);

    }

    if (cents >= 5){
        int cents5 = cents - cents%5;
        p1.nicks += cents5/5;
        cents = cents - (p1.nicks * 5);
